...
(function(){
  ...
  function show(v){const h=normalize(v),el=getEl();if(h===null||!el)return;magneticHeading=h;const text=`${h}° (${dir(h)})`;if(el.textContent!==text)el.textContent=text;el.dataset.compass='live'}
  ...
  function bind(){const el=getEl(),card=el?.closest('.compass-card');if(card){card.addEventListener('click',activate);card.addEventListener('touchend',activate,{passive:true});card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' ')activate()})}if(typeof DeviceOrientationEvent!=='undefined'&&typeof DeviceOrientationEvent.requestPermission!=='function')start();if(el){new MutationObserver(()=>{if(magneticHeading&&document.documentElement.dataset.compassActive==='true'){const text=`${magneticHeading}° (${dir(magneticHeading)})`;if(el.textContent!==text)el.textContent=text}}).observe(el,{childList:true,characterData:true,subtree:true})}if(el&&!active)el.textContent='Tap to activate'}
